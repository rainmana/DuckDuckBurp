...
DuckDuckBurp is a Burp Suite extension that fronts Burp proxy data with DuckDB to enable fast SQL querying and AI integration over HTTP traffic.

## Architecture

The extension lives in `ExtensionTemplateProject/` and is built as a fat JAR loaded into Burp Suite at runtime.

- **Entry point**: `ExtensionTemplateProject/src/main/java/Extension.java` — implements `BurpExtension`, receives a `MontoyaApi` instance via `initialize()`
...
- **JAR packaging**: Fat JAR via `tasks.named<Jar>("jar")` — bundles runtime classpath (for DuckDB JDBC or other runtime deps)

The Montoya API is the sole bridge to Burp. All Burp interaction goes through the `MontoyaApi` object passed to `initialize()`.

## Build Commands

All commands run from `ExtensionTemplateProject/`:

```bash
./gradlew jar      # Build the extension JAR → build/libs/extension-template-project.jar
...
- GUI elements must be parented to the Burp main frame
